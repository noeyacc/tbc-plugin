...
function SystemModule:Refresh()
  ...
  self.fpsText:SetText('000 fps') -- get the widest we can be
  local fpsWidest = self.fpsText:GetStringWidth()
  ...
  if db.modules.system.showWorld then
	self.worldPingText:SetText('000 ms')
  else
	if self.worldPing then
		self.worldPingText:SetText('')
	end
  end
  self.pingText:SetText('000 ms') -- get the widest we can be
...
function SystemModule:UpdateTexts()
  ...
  self.fpsText:SetText(floor(GetFramerate()).." fps")
  local _, _, homePing, worldPing = GetNetStats()
  self.pingText:SetText(L['L']..": "..floor(homePing).." ms")
  if xb.db.profile.modules.system.showWorld then
	self.worldPingText:SetText(L['W']..": "..floor(worldPing).." ms")
  end
end
