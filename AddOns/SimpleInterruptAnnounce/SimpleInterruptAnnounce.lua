...
function frame:COMBAT_LOG_EVENT_UNFILTERED()
	local timeStamp, subEvent, _, _, sourceName, sourceFlags, _, _, destName, _, destRaidFlags, _, spellName, _, _, extraSpellName = CombatLogGetCurrentEventInfo()
	...
	if sourceName == GetPlayerName() then
		...
		-- Announce interrupt
		self:AnnounceInterrupt(mode, string.format('已打斷 %s%s [%s]', GetRaidIconString(raidIcon, mode), StringOwns(destName or '?'), (extraSpellName or '?')), cfg.soundOwn)
	elseif bit.band(sourceFlags, COMBATLOG_OBJECT_AFFILIATION_MINE) ~= 0 then
		...
		-- Announce interrupt
		self:AnnounceInterrupt(mode, string.format('我的 %s 已打斷 %s%s [%s]', sourceName or '?', GetRaidIconString(raidIcon, mode), StringOwns(destName or '?'), (extraSpellName or '?')), cfg.soundOwn)
	else
		...
		if (inRaid and bit.band(sourceFlags, COMBATLOG_OBJECT_AFFILIATION_RAID) ~= 0) or (inGroup and bit.band(sourceFlags, COMBATLOG_OBJECT_AFFILIATION_PARTY) ~= 0) then
			...
			self:AnnounceInterrupt(mode, string.format('%s 已打斷 %s%s [%s]', name, GetRaidIconString(raidIcon, mode), StringOwns(destName or '?'), (extraSpellName or '?')), cfg.soundOther)
		end
	end
end
...
function frame:Print(...)
	return print('|cFFF0F050[斷法]|r:', ...)
end

-- Formatted print with SIA prefix
function frame:Printf(s, ...)
	return print('|cFFF0F050[斷法]|r:', string.format(s, ...))
end
...
function frame:OnSlashCmd(cmd, ...)
	...
	elseif cmd == 'help' then
		...
		self:Print('自己模式包含了你或你的寵物所做的斷法，')
		self:Print('他人模式包含了你的隊伍/團隊中的其他玩家或寵物所做的斷法。')
		self:Print('範例:')
		self:Print('  |cFF20E020/sia party|r - 顯示目前的隊伍模式。')
		self:Print('  |cFF20E020/sia solo self|r - 單人時，顯示自己的斷法給自己看。')
		self:Print('  |cFF20E020/sia raid say self|r - 團隊時，自己的斷法通報到說頻道，他人的只顯示給自己看。')
	else
		-- By default, show status and options
		self:Print(addonName, addon.version)
		self:Print('用法: /sia <指令> [選項]')
